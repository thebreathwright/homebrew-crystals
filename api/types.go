...
type GenerateRequest struct {
	...
	// RoutingBundle is an optional Brudo/Grax content-addressed crystal route.
	// The server consumes and receipts the route; it is not model prose.
	RoutingBundle map[string]any `json:"routing_bundle,omitempty"`

	// Sloom is an optional Brudo execution-context request. The server, rather
	// than model prose, determines and returns the corresponding receipt.
	Sloom map[string]any `json:"sloom,omitempty"`
}
...
type GenerateResponse struct {
	...
	// CrystalRuntime reports the deterministic crystal route consumed by the
	// executor, independently of the model-generated response text.
	CrystalRuntime map[string]any `json:"crystal_runtime,omitempty"`

	// SloomReceipt is a base64url-encoded, self-hashed daemon execution receipt.
	SloomReceipt string `json:"_brullama_transport_sloom_receipt,omitempty"`
}
