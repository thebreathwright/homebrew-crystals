...
// consumeCrystalRoute is the daemon-side boundary for crystal-native Grax.
// It deliberately consumes only a compact, content-addressed route summary;
// Brullama does not treat model prose as evidence or authority.
func consumeCrystalRoute(bundle map[string]any) (map[string]any, bool, error) {
	if bundle == nil {
		return nil, false, nil
	}
	if bundle["schema_version"] != "brudo-brullama-grax-routing-bundle-v1" {
		return nil, false, fmt.Errorf("unsupported crystal routing bundle schema")
	}
	projection, ok := bundle["lookup_projection_sha256"].(string)
	if !ok || len(projection) != 64 {
		return nil, false, fmt.Errorf("crystal route lacks lookup projection SHA-256")
	}
	registry, ok := bundle["crystal_registry_sha256"].(string)
	if !ok || len(registry) != 64 {
		return nil, false, fmt.Errorf("crystal route lacks registry SHA-256")
	}
	route, ok := bundle["listening_route"].(string)
	if !ok || (route != "CORE_ONLY" && route != "HARD_LISTEN") {
		return nil, false, fmt.Errorf("crystal route is invalid")
	}
	for _, field := range []string{"authority_inheritance", "canon_effect", "truth_effect", "memory_write_effect"} {
		if bundle[field] != "NONE" {
			return nil, false, fmt.Errorf("crystal route has forbidden effect: %s", field)
		}
	}
	crystals, ok := bundle["triggered_crystals"].([]any)
	if !ok {
		return nil, false, fmt.Errorf("crystal route triggered_crystals is not an array")
	}
	for _, crystal := range crystals {
		if _, ok := crystal.(string); !ok {
			return nil, false, fmt.Errorf("crystal route contains non-string crystal identity")
		}
	}
	context, consumedPackages, setHash, composition, err := consumeCrystalPackages(bundle)
	if err != nil {
		return nil, false, err
	}
	status := "CONSUMED"
	if composition["status"] != nil && composition["status"] != "CONSUMED" {
		status = composition["status"].(string)
	}
	return map[string]any{
		"schema_version":            "brullama-crystal-runtime-receipt-v1",
		"status":                    status,
		"crystal_registry_sha256":   registry,
		"lookup_projection_sha256":  projection,
		"listening_route":           route,
		"triggered_crystals":        crystals,
		"consumed_crystal_packages": consumedPackages,
		"crystal_set_sha256":        setHash,
		"crystal_context_sha256":    sha256Hex([]byte(context)),
		"crystal_composition":       composition,
		"_crystal_context":          context,
		"authority_inheritance":     "NONE",
		"canon_effect":              "NONE",
	}, route == "HARD_LISTEN", nil
}
...
func consumeCrystalPackages(bundle map[string]any) (string, []any, string, map[string]any, error) {
	set, ok := bundle["crystal_set"].(map[string]any)
	if !ok {
		return "", nil, "", map[string]any{"status": "NO_SET"}, nil
	}
	if set["schema"] != "brudo-grax-crystal-set-v1" || set["registry_schema"] != "brudo-grax-crystal-registry-v1" || set["admission_status"] != "ADMITTED" {
		return "", nil, "", nil, fmt.Errorf("crystal set is not an admitted canonical package set")
	}
	registry, ok := set["registry_sha256"].(string)
	if !ok || registry != bundle["crystal_registry_sha256"] {
		return "", nil, "", nil, fmt.Errorf("crystal set registry identity differs")
	}
	packages, ok := set["packages"].([]any)
	if !ok {
		return "", nil, "", nil, fmt.Errorf("crystal set packages are not an array")
	}
	triggeredRaw, _ := bundle["triggered_crystals"].([]any)
	triggered := make([]string, 0, len(triggeredRaw))
	for _, raw := range triggeredRaw {
		triggered = append(triggered, raw.(string))
	}
	...
	allPackages := make(map[string]map[string]any, len(packages))
	allReceipts := make(map[string]any, len(packages))
	allContext := make(map[string]string, len(packages))
	for _, raw := range packages {
		packageValue, ok := raw.(map[string]any)
		if !ok {
			return "", nil, "", nil, fmt.Errorf("crystal package is not an object")
		}
		id, ok := packageValue["crystal_id"].(string)
		if !ok || !slices.Contains([]string{"zircon", "moonstone", "alexandrite", "spinel", "itzelf", "obsidian"}, id) || seen[id] {
			return "", nil, "", nil, fmt.Errorf("crystal package id is invalid or duplicated")
		}
		seen[id] = true
		content, ok := packageValue["content"].(map[string]any)
		if !ok || content["schema"] != "brudo-grax-crystal-package-v1" || content["crystal_id"] != id || content["admission_status"] != "ADMITTED" {
			return "", nil, "", nil, fmt.Errorf("crystal package content is not admitted")
		}
		if content["registry_sha256"] != registry || content["truth_effect"] != "NONE" || content["authority_effect"] != "NONE" || content["canon_effect"] != "NONE" {
			return "", nil, "", nil, fmt.Errorf("crystal package has an effect or registry mismatch")
		}
		contentBytes, err := json.Marshal(content)
		if err != nil {
			return "", nil, "", nil, fmt.Errorf("crystal package content cannot be serialized: %w", err)
		}
		contentHash, ok := packageValue["content_sha256"].(string)
		if !ok || contentHash != sha256Hex(contentBytes) {
			return "", nil, "", nil, fmt.Errorf("crystal package content hash differs")
		}
		knowledgeContent := map[string]any{}
		for key, value := range content {
			if key != "licensing_status" {
				knowledgeContent[key] = value
			}
		}
		knowledgeBytes, err := json.Marshal(knowledgeContent)
		if err != nil {
			return "", nil, "", nil, fmt.Errorf("crystal knowledge projection cannot be serialized: %w", err)
		}
		licensing, _ := content["licensing_status"].(string)
		if !slices.Contains([]string{"UNASSESSED", "FREE", "LICENSED", "PRIVATE"}, licensing) {
			return "", nil, "", nil, fmt.Errorf("crystal package licensing status is invalid")
		}
		ids = append(ids, id)
		allPackages[id] = map[string]any{"operator": crystalOperator(id)}
		allContext[id] = fmt.Sprintf("CRYSTAL_PACKAGE %s %s\n%s", id, sha256Hex(knowledgeBytes), string(knowledgeBytes))
		allReceipts[id] = map[string]any{"crystal_id": id, "content_sha256": contentHash, "content_bytes_sha256": sha256Hex(contentBytes), "knowledge_bytes_sha256": sha256Hex(knowledgeBytes), "licensing_status": licensing}
	}
	setHash, ok := set["set_sha256"].(string)
	allReceiptList := make([]any, 0, len(ids))
	for _, id := range ids {
		allReceiptList = append(allReceiptList, allReceipts[id])
	}
	if !ok || setHash != crystalSetHash(registry, ids, allReceiptList) {
		return "", nil, "", nil, fmt.Errorf("crystal set content hash differs")
	}
	applicable := make([]string, 0, len(ids))
	for _, id := range ids {
		if triggeredSet[id] {
	...
	for index, left := range applicable {
		for _, right := range applicable[index+1:] {
			if allPackages[left]["operator"] == allPackages[right]["operator"] {
				conflicts = append(conflicts, left+"/"+right)
			}
		}
	}
	composition := map[string]any{
		"schema":              "brullama-crystal-composition-receipt-v1",
		"selected_crystals":   ids,
		"triggered_crystals":  triggered,
		"applicable_crystals": applicable,
		"conflicts":           conflicts,
		"rule":                "applicable = admitted ∩ triggered; same-operator overlap remains unresolved",
	}
	if len(conflicts) > 0 {
		composition["status"] = "WITHHELD_UNRESOLVED_CONFLICT"
		composition["reason"] = "same canonical crystal operator has overlapping applicability"
		return "", []any{}, setHash, composition, nil
	}
	consumed := make([]any, 0, len(applicable))
	contextParts := make([]string, 0, len(applicable))
	for _, id := range applicable {
		consumed = append(consumed, allReceipts[id])
		contextParts = append(contextParts, allContext[id])
	}
	if len(applicable) == 0 {
		composition["status"] = "NARROWED_EMPTY"
		composition["reason"] = "no admitted package was present in the observed route"
	} else if len(applicable) < len(ids) {
		composition["status"] = "NARROWED"
		composition["reason"] = "irrelevant admitted packages were excluded before composition"
	} else {
		composition["status"] = "CONSUMED"
	}
	return strings.Join(contextParts, "\n\n"), consumed, setHash, composition, nil
}

func crystalOperator(id string) string {
	operators := map[string]string{
		"zircon": "distinction", "moonstone": "authority_shift", "alexandrite": "failure_repair",
		"spinel": "counterexample", "itzelf": "failure_repair", "obsidian": "transformation_invariance",
	}
	return operators[id]
}

func crystalSetHash(registry string, ids []string, receipts []any) string {
	rows := make([]string, 0, len(receipts))
	for index, raw := range receipts {
		row := raw.(map[string]any)
		rows = append(rows, fmt.Sprintf("%s:%s", ids[index], row["content_sha256"].(string)))
	}
	slices.Sort(rows)
	return sha256Hex([]byte(registry + "|" + strings.Join(rows, "|")))
}
...
func brullamaIdentity() gin.H {
	mlxParallelism := max(int(envconfig.NumParallel()), 1)
	mlxExecution := "isolated_process_pool"
	if mlxParallelism == 1 {
		mlxExecution = "serialized"
	}
	return gin.H{
		"product":                   "brullama",
		"daemon":                    true,
		"version":                   version.Version,
		"upstream":                  "Ollama v0.32.4",
		"upstream_commit":           "64ee2f9847ccaedd8f05a139c30f086e9e0abe73",
		"source_archive_sha256":     "c20ff44d18453311f0d724e9215336add75a7c01efed2434a409b5b6715e49f3",
		"sloom_context_version":     "sloom-listening-differential-v2",
		"dedicated_body":            true,
		"silent_truncation_allowed": false,
		"listening_differential":    true,
		"variable_map_slice":        true,
		"mlx_request_execution":     mlxExecution,
		"mlx_max_active_requests":   mlxParallelism,
		"usage":                     brullamaUsageSnapshot(),
	}
}
...
func (s *Server) GenerateHandler(c *gin.Context) {
	...
	sloomReceipt, err := consumeSloomContext(req.Sloom, req.Options)
	if err != nil {
		c.AbortWithStatusJSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}

	crystalRuntime, crystalActive, err := consumeCrystalRoute(req.RoutingBundle)
	if err != nil {
		c.AbortWithStatusJSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
	if crystalContext, ok := crystalRuntime["_crystal_context"].(string); ok && crystalContext != "" {
		delete(crystalRuntime, "_crystal_context")
		req.Prompt = crystalContext + "\n\n" + req.Prompt
	}
	if crystalActive {
		if req.Options == nil {
			req.Options = map[string]any{}
		}
		// HARD_LISTEN is an observable deterministic runtime effect: bound the
		// decode budget before MLX execution. The receipt exposes the effective
		// value separately from the model's prose.
		if requested, ok := req.Options["num_predict"].(float64); ok && requested > 512 {
			req.Options["num_predict"] = float64(512)
			crystalRuntime["effective_num_predict"] = 512
			crystalRuntime["decode_budget_effect"] = "HARD_LISTEN_CAP_512"
		}
	}
	...
	if !req.Raw {
		...
		if values.Messages != nil && values.Suffix == "" && req.Template == "" {
			if m.IsMLX() && len(req.Format) > 0 {
				slog.Warn("MLX structured output uses prompt guidance, not constrained decoding")
				values.Messages = withMLXFormatRequest(values.Messages, req.Format)
			}
	...
	go func() {
		...
		defer close(ch)
		endBrullamaExecution := beginBrullamaExecution()
		defer endBrullamaExecution()
		if err := r.Completion(c.Request.Context(), llm.CompletionRequest{
			Prompt:          prompt,
			RequestID:       c.GetHeader("X-Brudo-Request-ID"),
			Media:           media,
			Format:          req.Format,
			Options:         opts,
			Shift:           req.Shift == nil || *req.Shift,
			...
			res := api.GenerateResponse{
				...
				CrystalRuntime: crystalRuntime,
				SloomReceipt:   sloomReceipt,
			}
			...
			if cr.Done {
				if crystalActive {
					crystalRuntime["deterministic_decision"] = crystalDeterministicDecision(res.Response)
				}
				recordBrullamaUsage(cr.PromptEvalCount, cr.EvalCount)
	...
	if req.Stream != nil && !*req.Stream {
		...
		if crystalActive {
			crystalRuntime["deterministic_decision"] = crystalDeterministicDecision(r.Response)
			r.CrystalRuntime = crystalRuntime
		}
...
func (s *Server) GenerateRoutes() (http.Handler, error) {
	...
	r.HEAD("/api/brullama", func(c *gin.Context) { c.JSON(http.StatusOK, brullamaIdentity()) })
	r.GET("/api/brullama", func(c *gin.Context) { c.JSON(http.StatusOK, brullamaIdentity()) })
...
func (s *Server) ChatHandler(c *gin.Context) {
	...
	if m.IsMLX() && len(req.Format) > 0 {
		slog.Warn("MLX structured output uses prompt guidance, not constrained decoding")
		msgs = withMLXFormatRequest(msgs, req.Format)
	}
